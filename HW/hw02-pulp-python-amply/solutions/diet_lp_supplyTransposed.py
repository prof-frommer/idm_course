...
def diet_lp(dat_file_name):
    ...
    set foodItems;
    set nutrients;
    param objective{foodItems};
    param reqs{nutrients};
    param supply{nutrients,foodItems};
    param maxForItem{foodItems};
    """)
    ...
    for nutr in data.nutrients:
       ...
       for food in data.foodItems:
           print('supply ',nutr,food,data.supply[nutr][food])
           
    # Dec vars       
    x = LpVariable.dicts('x',data.foodItems,0)
    
    # objective
    diet_lp_model += lpSum(data.objective[i] * x[i] for i in data.foodItems)
    
    # constraints - loop through reqs adding an lpSum for each
    for i in data.nutrients:
        constr_LHS = lpSum(data.supply[i][j] * x[j] for j in data.foodItems)
        diet_lp_model += constr_LHS >= data.reqs[i], i
            
    # upper limits
    for i in data.foodItems:
        diet_lp_model += x[i] <= data.maxForItem[i], i        
